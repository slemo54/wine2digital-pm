...
export async function PATCH(req: NextRequest, { params }: { params: { clientId: string } }): Promise<NextResponse> {
  try { const auth = await getClockifyV2CatalogActor(); if (!auth.actor) return auth.response!; return NextResponse.json({ client: await updateClockifyClient(prisma, auth.actor, params.clientId, await req.json()) }); }

...
export async function POST(_: Request, { params }: { params: { projectId: string; taskId: string } }): Promise<NextResponse> {
  try { const auth = await getClockifyV2CatalogActor(); if (!auth.actor) return auth.response!; return NextResponse.json({ task: await updateClockifyTask(prisma, auth.actor, params.projectId, params.taskId, { isActive: false }) }); }

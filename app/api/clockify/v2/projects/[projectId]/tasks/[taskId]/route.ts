...
export async function PATCH(req: NextRequest, { params }: { params: { projectId: string; taskId: string } }): Promise<NextResponse> {
  try { const auth = await getClockifyV2CatalogActor(); if (!auth.actor) return auth.response!; return NextResponse.json({ task: await updateClockifyTask(prisma, auth.actor, params.projectId, params.taskId, await req.json()) }); }

...
export async function PUT(req: NextRequest, { params }: { params: { projectId: string } }): Promise<NextResponse> {
  try { const auth = await getClockifyV2CatalogActor(); if (!auth.actor) return auth.response!; const body = await req.json(); return NextResponse.json({ project: await setClockifyProjectManager(prisma, auth.actor, params.projectId, body.managerId) }); }

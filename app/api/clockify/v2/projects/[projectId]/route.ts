...
export async function PATCH(req: NextRequest, { params }: { params: { projectId: string } }): Promise<NextResponse> {
  try { const auth = await getClockifyV2CatalogActor(); if (!auth.actor) return auth.response!; return NextResponse.json({ project: await updateClockifyProject(prisma, auth.actor, params.projectId, await req.json()) }); }

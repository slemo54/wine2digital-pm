...
export async function POST(_: Request, { params }: { params: { projectId: string } }): Promise<NextResponse> {
  try { const auth = await getClockifyV2CatalogActor(); if (!auth.actor) return auth.response!; return NextResponse.json({ project: await setClockifyProjectArchive(prisma, auth.actor, params.projectId, false) }); }

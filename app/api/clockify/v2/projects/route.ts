...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try { const auth = await getClockifyV2CatalogActor(); if (!auth.actor) return auth.response!; return NextResponse.json({ project: await createClockifyProject(prisma, auth.actor, await req.json()) }, { status: 201 }); }

...
export async function POST(req: NextRequest, { params }: { params: { entryId: string } }): Promise<NextResponse> {
  try { const auth = await getClockifyV2Actor(); if (!auth.actor) return auth.response!; const body = await parseClockifyV2Json(req); return NextResponse.json(await splitClockifyEntry(prisma, auth.actor, params.entryId, body.splitAt)); }

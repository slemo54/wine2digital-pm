...
export default function DashboardPage() {
  ...
  // Performance: Memoize work items and their derived statuses to avoid recalculations on every render
  const now = useMemo(() => new Date(), []);

  const isOverdue = useMemo(() => {
    return (t: { dueDate: string | null; status: string }) =>
  ...
  const workItems = useMemo(() => [
    ...(tasks?.tasks || []).map((t: TaskListItem) => ({
      kind: "task" as const,
      taskId: t.id,
    ...
    ...(subtasks?.subtasks || []).map((s: SubtaskListItem) => ({

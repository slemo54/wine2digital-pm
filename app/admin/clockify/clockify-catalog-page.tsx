...
type Task = { id: string; name: string; isActive: boolean };
type Project = { id: string; name: string; client: string; clientId: string | null; color: string; isActive: boolean; managerId: string | null; manager: Manager | null; origin: string };
...
export default function ClockifyCatalogPage({ role }: { role: "admin" | "manager" }): JSX.Element {
  ...
  async function createClient(): Promise<void> {
    const name = window.prompt("Nome del nuovo cliente"); if (!name?.trim()) return;
  ...
  async function addTask(event: FormEvent): Promise<void> {
    event.preventDefault(); if (!taskProject) return;
    try { const data = await request<{ task: Task }>(`/api/clockify/v2/projects/${taskProject.id}/tasks`, { method: "POST", body: JSON.stringify({ name: taskName }) }); setTasks((current) => [...current, data.task]); setTaskName(""); }
    catch (error) { toast.error(error instanceof Error ? error.message : "Errore task"); }
  }
  async function toggleTask(task: Task): Promise<void> {
    if (!taskProject) return;
    try { const data = await request<{ task: Task }>(`/api/clockify/v2/projects/${taskProject.id}/tasks/${task.id}`, { method: "PATCH", body: JSON.stringify({ isActive: !task.isActive }) }); setTasks((current) => current.map((item) => item.id === task.id ? data.task : item)); }
    catch (error) { toast.error(error instanceof Error ? error.message : "Errore task"); }
  }
  ...
  return <main className="min-h-screen bg-secondary"><div className="mx-auto max-w-6xl space-y-6 px-4 py-6 sm:px-6 sm:py-8">
    ...
    <Tabs value={tab} onValueChange={setTab}><TabsList aria-label="Sezioni catalogo Clockify"><TabsTrigger value="active">Progetti attivi</TabsTrigger><TabsTrigger value="archive">Archivio</TabsTrigger><TabsTrigger value="report">Report</TabsTrigger></TabsList>
      ...
      <TabsContent value="active"><CatalogList title={title} q={q} setQ={setQ} loading={loading} projects={projects} onCreate={showCreate} onEdit={showEdit} onTasks={openTasks} onArchive={(project) => archive(project, true)} /></TabsContent>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent><DialogHeader><DialogTitle>{editing ? "Modifica progetto" : "Nuovo progetto"}</DialogTitle></DialogHeader><form className="space-y-4" onSubmit={saveProject}>
      ...
      <div><Label htmlFor="project-client">Cliente</Label><div className="flex gap-2"><Select value={form.clientId} onValueChange={(clientId) => setForm({ ...form, clientId })}><SelectTrigger id="project-client"><SelectValue placeholder="Scegli cliente" /></SelectTrigger><SelectContent>{clients.map((client) => <SelectItem key={client.id} value={client.id}>{client.name}</SelectItem>)}</SelectContent></Select><Button type="button" variant="outline" onClick={createClient} aria-label="Crea un nuovo cliente"><Plus className="h-4 w-4" /></Button></div></div>
    ...
    <Dialog open={!!taskProject} onOpenChange={(next) => { if (!next) setTaskProject(null); }}><DialogContent><DialogHeader><DialogTitle>Attività · {taskProject?.name}</DialogTitle></DialogHeader><form className="flex gap-2" onSubmit={addTask}><Input aria-label="Nuova attività" required value={taskName} onChange={(e) => setTaskName(e.target.value)} placeholder="Nuova attività" /><Button type="submit"><Plus className="mr-1 h-4 w-4" />Aggiungi</Button></form><ul className="max-h-64 space-y-2 overflow-auto" aria-label="Attività progetto">{tasks.map((task) => <li key={task.id} className="flex items-center justify-between rounded border p-2"><span className={task.isActive ? "" : "text-muted-foreground line-through"}>{task.name}</span><Button variant="outline" size="sm" onClick={() => toggleTask(task)}>{task.isActive ? "Disattiva" : "Riattiva"}</Button></li>)}</ul></DialogContent></Dialog>
  </div></main>;
}

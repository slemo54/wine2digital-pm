...
import { ClockifyLocksPanel } from "./clockify-locks-panel";
...
export default function ClockifyCatalogPage({ role }: { role: "admin" | "manager" }): JSX.Element {
  ...
  async function createClient(suggestedName?: string): Promise<void> {
    const name = suggestedName?.trim(); if (!name) { toast.error("Inserisci il nome del cliente"); throw new Error("Nome cliente richiesto"); }
    ...
    catch (error) { toast.error(error instanceof Error ? error.message : "Errore cliente"); throw error; }
  }
  ...
  async function createTask(name: string): Promise<void> {
    ...
    catch (error) { toast.error(error instanceof Error ? error.message : "Errore task"); throw error; }
  }
  ...
  return <main className="min-h-screen bg-secondary"><div className="mx-auto max-w-6xl space-y-6 px-4 py-6 sm:px-6 sm:py-8">
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent><DialogHeader><DialogTitle>{editing ? "Modifica progetto" : "Nuovo progetto"}</DialogTitle></DialogHeader><form className="space-y-4" onSubmit={saveProject}>
      ...
      <div><Label htmlFor="project-client">Cliente</Label><ClientCombobox inputId="project-client" clients={clients} value={form.clientId} onChange={(clientId) => setForm({ ...form, clientId })} onCreate={(name) => void createClient(name)} /></div>
      <div className="space-y-2"><Label htmlFor="project-color">Colore progetto</Label><div className="flex items-center gap-3"><Input id="project-color" type="color" className="h-11 w-16 cursor-pointer p-1" required value={form.color} onChange={(e) => setForm({ ...form, color: e.target.value.toUpperCase() })} /><span className="text-sm text-muted-foreground">Usato nel timesheet, nel calendario e nei report.</span></div></div>
...
function CatalogList({ title, q, setQ, loading, projects, onCreate, onEdit, onTasks, onArchive, archived = false }: { title: string; q: string; setQ: (value: string) => void; loading: boolean; projects: Project[]; onCreate: () => void; onEdit: (project: Project) => void; onTasks: (project: Project) => void; onArchive: (project: Project) => void; archived?: boolean }): JSX.Element {
  return <Card><CardHeader className="gap-3 sm:flex-row sm:items-center sm:justify-between"><CardTitle>{title}</CardTitle><Button onClick={onCreate}><Plus className="mr-1 h-4 w-4" />Nuovo progetto</Button></CardHeader><CardContent className="space-y-4"><Input value={q} onChange={(event) => setQ(event.target.value)} placeholder="Cerca per progetto o cliente" aria-label="Cerca progetti" />{loading ? <div className="flex items-center gap-2 py-8 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Caricamento…</div> : <div className="grid gap-3 lg:grid-cols-2">{projects.map((project) => <article key={project.id} className="flex min-h-[154px] flex-col justify-between gap-4 rounded-xl border bg-card p-4"><div><div className="flex items-start gap-3"><span className="mt-1 h-3 w-3 shrink-0 rounded-full" style={{ backgroundColor: project.color }} aria-hidden /><div className="min-w-0"><h3 className="truncate text-lg font-semibold">{project.name}</h3><p className="text-sm text-muted-foreground">{project.client || "Senza cliente"}</p></div></div><div className="mt-3 flex flex-wrap gap-2"><Badge variant={archived ? "warning" : "success"}>{archived ? "Archiviato" : "Attivo"}</Badge><Badge variant="secondary">{project.origin === "imported" ? "Importato" : "Manuale"}</Badge><Badge variant="outline">{project._count?.tasks || 0} task</Badge></div><p className="mt-3 text-sm text-muted-foreground">Responsabile: <span className="font-medium text-foreground">{project.manager ? project.manager.name || project.manager.email : "Non assegnato"}</span></p></div><div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={() => onTasks(project)}>Gestisci task</Button><Button variant="outline" size="sm" onClick={() => onEdit(project)}>Modifica</Button><Button variant="outline" size="sm" onClick={() => onArchive(project)}>{archived ? <><RotateCcw className="mr-1 h-4 w-4" />Ripristina</> : <><Archive className="mr-1 h-4 w-4" />Archivia</>}</Button></div></article>)}{projects.length === 0 && <p className="py-8 text-center text-sm text-muted-foreground lg:col-span-2">Nessun progetto trovato.</p>}</div>}</CardContent></Card>;
}

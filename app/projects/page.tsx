...
export default function ProjectsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const avgCompletion =
      projects.length > 0
        ? Math.round(
            projects.reduce((acc, p) => acc + p.completionRate, 0) /
              projects.length,
          )
        : 0;
    const finishedTasks = projects.reduce(
      (acc, p) => acc + p.tasksCompleted,
      0,
    );
  ...
  const getMyProjectRole = (project: Project): string | null => {
    ...
    const membership = Array.isArray(project.members)
      ? project.members.find(
          (m) => m?.userId === meId || m?.user?.id === meId,
        ) || null
      : null;
  ...
  const getPermissionTooltip = (action: "edit" | "archive" | "delete") => {
    if (action === "delete")
      return "Solo admin, creator o owner del progetto possono eliminare.";
    if (action === "archive")
      return "Solo admin, creator o owner/manager del progetto possono archiviare.";
    ...
    return (
      <Tooltip>
        <TooltipTrigger asChild>
          <span
            className={
              disabled ? "inline-flex cursor-not-allowed" : "inline-flex"
            }
          >
            <Button
              type="button"
  ...
  const bulkAction = async (action: "archive" | "delete") => {
    ...
    if (action === "delete") {
      const ok = confirm(
        `Eliminare definitivamente ${selectedProjects.size} progetti?`,
      );
      if (!ok) return;
    }
    ...
    try {
      ...
      const requested =
        Number((data as any)?.requested ?? selectedProjects.size) ||
        selectedProjects.size;
      const processed =
        Number(
          (data as any)?.processed ??
      ...
      toast.error(
        err instanceof Error ? err.message : "Impossibile completare l'azione",
      );
    } finally {
  ...
  const archiveSingleProject = async (project: Project) => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(String((data as any)?.error || "Archive failed"));
  ...
  const exportProjects = async (format: "csv" | "json") => {
    ...
    if (total > MAX_EXPORT) {
      toast.error(
        `Troppi progetti da esportare (${total}). Applica filtri e riprova.`,
      );
      return;
    }
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok)
        throw new Error(String((data as any)?.error || "Export failed"));
      const rows = Array.isArray((data as any)?.projects)
      ...
      const members = (p: Project) =>
        Array.isArray(p.members) ? p.members : [];
      const owners = (p: Project) =>
        members(p)
          .filter((m) => m?.role === "owner")
          .map(getEmail)
          .filter(Boolean);
      const managers = (p: Project) =>
        members(p)
          .filter((m) => m?.role === "manager")
          .map(getEmail)
          .filter(Boolean);
      const memberEmails = (p: Project) =>
        members(p)
  ...
  const sortIcon = (col: OrderBy) => {
    if (orderBy !== col)
      return <SortAsc className="h-3.5 w-3.5 text-gray-400" />;
    return order === "asc" ? (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Manage your projects and their real-time progress efficiently.
            </p>
          </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  disabled={
                    isExporting || (pagination?.total ?? projects.length) === 0
                  }
                >
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem
                  onClick={() => void exportProjects("csv")}
                  disabled={isExporting}
                >
                  Export CSV
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => void exportProjectsXls()}
                  disabled={isExporting}
                >
                  Export XLS
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => void exportProjects("json")}
                  disabled={isExporting}
                >
                  Export JSON
                </DropdownMenuItem>
                    ...
                    <table className="w-full caption-bottom text-sm">
                      ...
                      <tbody className="[&_tr:last-child]:border-0">
                        {projects.map((project) => {
                          const perms = getProjectPermissions(project);
                          const editTooltip = perms.canEdit
                            ? "Modifica progetto"
                            : getPermissionTooltip("edit");
                          ...
                          const deleteTooltip = perms.canDelete
                            ? "Elimina progetto"
                            : getPermissionTooltip("delete");
                            ...
                            >
                              ...
                              <td className="p-4 align-middle min-w-[180px]">
                                <div className="flex items-center gap-3">
                                  ...
                                  <span className="text-xs font-bold text-foreground w-[3ch] text-right">
                                  ...
                                  {project.members
                                    ...
                                    .map((member, idx) => (
                                      <Avatar
                                        ...
                                        style={{
                                          zIndex: project.members.length - idx,
                                        }}
                                      >
                              ...
                              <td className="p-4 align-middle text-right">
                                <div className="flex justify-end gap-1 opacity-60 hover:opacity-100 transition-opacity">
                                  ...
                                  <ActionIconButton
                                    ...
                                    onClick={() =>
                                      void archiveSingleProject(project)
                                    }
                                  >
                                    <Archive className="h-4 w-4" />
                                  </ActionIconButton>
                                  <ActionIconButton
                                    ...
                                    onClick={() =>
                                      void deleteSingleProject(project)
                                    }

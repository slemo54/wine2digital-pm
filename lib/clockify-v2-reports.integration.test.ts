...
test("PostgreSQL report engine: Rome filters, cursors, tag allocation, bounded groups, weekly paging, and live scope reduction", { skip: !url }, async () => {
  ...
  try {
    ...
    assert.equal(summary.totalMin, 36); assert.equal(summary.bar.reduce((sum: number, row: any) => sum + row.totalMin, 0), 36);
    ...
    const weeklyOne: any = await runClockifyReport(db, { userId: admin.id, role: "admin", department: null }, normalizeClockifyReportInput({ reportType: "weekly", from: "2026-03-29", to: "2026-03-29", limit: 1 }));
    assert.equal(weeklyOne.people[0].userId, blank.id); assert.ok(weeklyOne.nextCursor); assert.equal(weeklyOne.grandTotalMin, 32);
    const weeklyTwo: any = await runClockifyReport(db, { userId: admin.id, role: "admin", department: null }, normalizeClockifyReportInput({ reportType: "weekly", from: "2026-03-29", to: "2026-03-29", limit: 1, cursor: weeklyOne.nextCursor }));
    assert.notEqual(weeklyTwo.people[0].userId, blank.id); assert.equal(weeklyTwo.grandTotalMin, 32);

...
test("PostgreSQL audit excludes deleted records and controlled import is idempotent", { skip: !url }, async () => {
  ...
  try {
    ...
    const [user, manager, colleague, outside] = await Promise.all([
      ...
      db.user.create({ data: { email: `${marker}-manager@test.invalid`, role: "manager", department: "Grafica", isActive: true } }),
      db.user.create({ data: { email: `${marker}-colleague@test.invalid`, role: "member", department: "Grafica", isActive: true } }),
    ...
    const scopedFirst: any = await auditClockifyEntries(db, { userId: manager.id, role: "manager", department: "Grafica" }, normalizeClockifyAuditInput({ limit: 1 }));
    assert.equal(scopedFirst.entries.length, 1); assert.ok(scopedFirst.nextCursor); assert.equal(scopedFirst.entries[0].user.department, "Grafica");
    const scopedSecond: any = await auditClockifyEntries(db, { userId: manager.id, role: "manager", department: "Grafica" }, normalizeClockifyAuditInput({ limit: 1, cursor: scopedFirst.nextCursor }));
    assert.equal(scopedSecond.entries.length, 1); assert.equal(scopedSecond.entries[0].user.department, "Grafica"); assert.notEqual(scopedSecond.entries[0].id, scopedFirst.entries[0].id);
  } finally {

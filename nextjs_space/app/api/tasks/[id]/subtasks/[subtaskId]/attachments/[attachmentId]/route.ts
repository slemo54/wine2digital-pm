...
export async function DELETE(
  ...
  try {
    ...
    const existing = await prisma.subtaskAttachment.findUnique({
      ...
      select: {
        ...
        fileName: true,
        filePath: true,
        subtask: { select: { taskId: true } },
      },
    });
    if (!existing || existing.subtaskId !== params.subtaskId || existing.subtask.taskId !== params.id) {
    ...
    const canDelete =
      role === "admin" || isProjectManager || (role === "manager" && access.isProjectMember) || isUploader;
    if (!canDelete) return NextResponse.json({ error: "Insufficient permissions" }, { status: 403 });

    await prisma.subtaskAttachment.delete({ where: { id: params.attachmentId } });
